...
def function7():
    ...
    x = np.zeros(x.shape, dtype = 'int64')
...
def function12():
    ...
    arr = np.array([1,2,3])
    ans = np.vstack((arr,arr,arr))#write your code here 
    ans0= np.hstack(ans)
    ans1= np.repeat(arr,3)
    ans = np.concatenate([ans1,ans0])
...
def function18():
    ...
    x = len(students[np.isin(students,piaic)]) # Write you code Here
    return x
